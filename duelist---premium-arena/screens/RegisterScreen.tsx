...
export const RegisterScreen: React.FC<RegisterScreenProps> = ({ onNavigate, onRegister }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (email && name) onRegister(email, name);
  };

  return (
    <div class="flex flex-col h-screen w-full px-8 pt-16 pb-12 items-center justify-between">
      <div class="flex flex-col items-center w-full mt-4">
        <Logo />
        <h1 class="text-3xl font-serif font-bold tracking-tight text-white mb-2">Duelist</h1>
        <p class="text-zinc-500 text-sm font-light tracking-wide">Join the Arena of Choice</p>
      </div>
      ...
      <form onSubmit={handleSubmit} class="w-full space-y-4 max-w-sm">
        ...
        <div class="text-center pt-4">
          <p class="text-[10px] text-zinc-600 uppercase tracking-widest mb-4">By joining, you agree to the Rules of Engagement</p>
        </div>
      </form>
      ...
      <div class="w-full text-center space-y-6 max-w-sm">
        <div class="flex items-center gap-4 px-4">
          ...
          <span class="text-[10px] uppercase tracking-widest text-zinc-600 font-medium">Already Rank One</span>
          <div class="h-[0.5px] flex-1 bg-zinc-800"></div>
        </div>
        <button 

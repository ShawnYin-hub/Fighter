  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden select-none">
      <header className="w-full px-6 pt-10 pb-4 flex justify-between items-center z-50">
        <button onClick={() => navigate('/')} className="w-10 h-10 rounded-full glass flex items-center justify-center transition active:scale-95">
        ...
        <button className="w-10 h-10 rounded-full glass flex items-center justify-center">
          <span className="material-symbols-outlined text-2xl text-gray-400">share</span>
        </button>
      </header>
      ...
      <main className="flex-1 w-full overflow-y-auto no-scrollbar relative flex flex-col items-center">
        <div className="relative z-20 mt-8 mb-16 flex flex-col items-center">
          <div className="bg-white rounded-2xl w-48 py-8 px-4 flex flex-col items-center shadow-glow border border-win-gold/30">
            <div className="w-12 h-12 bg-dark-charcoal rounded-full flex items-center justify-center mb-4">
              <span className="material-symbols-outlined text-win-gold text-2xl">workspace_premium</span>
            </div>
        ...
        <div className="w-full max-w-md px-6 relative flex flex-col items-center pb-12">
          ...
          <div className="flex justify-between w-full mb-16 relative z-10">
            <div className="w-24 bg-dark-charcoal border border-gray-800 rounded-xl py-4 flex flex-col items-center opacity-40 grayscale scale-90">
              <span className="text-xs font-serif text-gray-500 mb-1">{decision.eliminated[0] || '...'}</span>
              <span className="text-[8px] tracking-tighter text-gray-600">Level 02</span>
            </div>
            <div className="w-24 bg-white rounded-xl py-4 flex flex-col items-center shadow-lg">
              <span className="text-xs font-serif text-black font-bold mb-1">{decision.winner}</span>
              <span className="text-[8px] tracking-tighter text-gray-400">Level 02</span>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-md px-6 mb-32 mt-4 relative z-20">
          <div className="glass rounded-2xl p-5 shadow-2xl border-l-4 border-l-win-gold bg-gradient-to-r from-win-gold/5 to-transparent">
            <div className="flex items-center gap-3 mb-3">
              <div className="p-1.5 bg-win-gold/10 rounded-lg">
                <span className="material-symbols-outlined text-win-gold text-lg block">temp_preferences_custom</span>
              </div>
              <span className="text-[10px] font-bold text-win-gold uppercase tracking-[0.2em]">{t.logic}</span>
            </div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 px-6 pb-10 pt-4 bg-gradient-to-t from-dark-charcoal via-dark-charcoal to-transparent max-w-md mx-auto z-50">
        <div className="w-full glass rounded-2xl py-4 px-6 flex items-center justify-between">
          <div className="flex flex-col">
            <span className="text-[10px] uppercase tracking-wider text-gray-500 font-medium mb-0.5">{t.efficiency}</span>
            <span className="text-lg font-serif font-bold text-white leading-none">{decision.efficiency || 88}% {t.clear}</span>
          </div>
          <button onClick={() => navigate('/history-dark')} className="bg-win-gold text-dark-charcoal px-6 py-2.5 rounded-full font-bold text-xs tracking-tight shadow-glow active:scale-95 transition-transform">
            {t.finish}
          </button>
        </div>
      </footer>

  ...
  return (
    <div className="w-full">
      ...
      <div className="w-full space-y-2 mb-12">
        <MenuItem icon="manage_accounts" label="Account" />
        <MenuItem icon="architecture" label="Decision Style" />
        <MenuItem icon="shield_lock" label="History Privacy" />
        <MenuItem icon="dark_mode" label="App Theme" rightText="Dark" />
      </div>

      {/* Sign Out */}
      <div className="py-8 text-center">
        <button className="text-[12px] font-serif uppercase tracking-[0.2em] text-duelist-gold hover:text-duelist-gold-light transition-colors px-6 py-2.5 border border-duelist-gold/20 rounded-lg active:scale-95 transition-all">
          Sign Out
        </button>
      </div>
    </div>
...
const MenuItem: React.FC<{ icon: string; label: string; rightText?: string }> = ({ icon, label, rightText }) => (
  <div className="w-full glass-row rounded-xl px-5 py-4 flex items-center justify-between group active:scale-[0.98] transition-all cursor-pointer">
